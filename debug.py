...
class Initialize():  # Runtime debug
    ...
    def test1(self):
        ...
        t = [queue.send('api.__init__', util.random_string()) for x in range(100)]
        ping = round((time.monotonic() - before) * 1000)
        [log.debug(x) for x in t]
        log.debug(f'Sent 100 messages in {ping}ms')
        # log.debug(t)
        # for x in range(100):
        #     queue.send('api.__init__', util.random_string())
        # time.sleep(60)
